...
class DHT(object):
    ...
    def start(self, seed_peer):
        ...
        ip = seed_peer.ip
        port = seed_peer.port
        self._add_known_node(
            ('tcp://%s:%s' % (ip, port), seed_peer.guid, seed_peer.nickname))
            ...
            if 'foundNode' in msg.keys():
                ...
                if foundNode[2] != self.transport.guid:
                    ...
                    if len(foundNode) == 3:
                        foundNode.append('')
                    self.add_peer(
                        foundNode[1], foundNode[2], foundNode[0], foundNode[3])
    ...
    def extendShortlist(self, findID, foundNodes):
        ...
        for node in foundNodes:
            ...
            if node_guid != self.settings['guid']:
                self.log.debug(
                    'Adding new peer to active peers list: %s', node)
    ...
    def find_listings_by_keyword(self, keyword, listingFilter=None,
                                 callback=None):
...
class DHTSearch(object):
    def __init__(self, market_id, key, call="findNode", callback=None):
        ...
        # If a findValue search is found this is the value:
