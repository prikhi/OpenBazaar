...
class CryptoTransportLayer(TransportLayer):
    ...
    def __init__(self, ob_ctx, db):
        ...
        if ((ob_ctx.bm_user, ob_ctx.bm_pass, ob_ctx.bm_port) !=
                (None, None, -1)):
        ...
        if (ob_ctx.enable_ip_checker and not ob_ctx.seed_mode
                and not ob_ctx.dev_mode):
    ...
    def _setup_settings(self):
        ...
        if not self.settings.get('PGPPubKey'):
            try:
                message = ('Generating PGP keypair. '
                           'This may take several minutes...')
                self.log.info(message)
                print message
                gpg = gnupg.GPG()
                input_data = gpg.gen_key_input(
                    key_type="RSA", key_length=2048, name_email='gfy@gfy.com',
                ...
                newsettings = {"PGPPubKey": pubkey_text,
        ...
        if not self.settings.get('nickname'):
            ...
            self.db.updateEntries('settings',
                                  newsettings, {"market_id": self.market_id})
            self.settings.update(newsettings)
        ...
        # In case user wants to override with command line passed bitmessage
        # values
    ...
    def respond_pubkey_if_mine(self, nickname, ident_pubkey):
        ...
        pubkey = self.cryptor.pubkey  # XXX: A Cryptor does not have this field
        ...
        if send_to is not None:
            ...
            if peer:
                ...
                try:
                    ...
                    self.log.error(
                        'Failed to send message directly to peer %s', e)
            else:
                self.log.warning(
